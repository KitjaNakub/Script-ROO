...
adminScriptList.addEventListener('click', (e) => {
    if (e.target.classList.contains('edit-btn')) {
        ...
        if (script) {
            ...
            // เซ็ตค่า Copy หรือ Link
            if (script.action === 'copy') {
                document.getElementById('edit-script-action-copy').checked = true;
            } else if (script.action === 'link') {
                document.getElementById('edit-script-action-link').checked = true;
            }
